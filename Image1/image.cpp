...
namespace Imaging
{
	...
	{
		ImageFrame::EvalSize(src.data.size(), src.size.width, src.size.height, src.depth);
	...
	{
		// TODO: Consider padding bytes.
		return GetNumBytes(this->dataType) * this->depth * this->size.width * pt.y +
			GetNumBytes(this->dataType) * this->depth * pt.x;
	}
	...
	{
		ImageFrame::EvalSize(srcData.size(), sz.width, sz.height, d);
	...
	any client. */
	void ImageFrame::EvalSize(SizeType length, SizeType w, SizeType h, SizeType d)
	{
		if (length != w * h * d)
		{
			std::ostringstream errMsg;
